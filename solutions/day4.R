solve_part1 <- function(boards, board_dim) {
    for (num in numbers) {
...
solve_part2 <- function(boards, board_dim = 5, numbers) {
    for (num in numbers) {
...
board_dim <- 5
boards <- array(t(as.matrix(raw_input)),
    dim = c(board_dim, board_dim, nrow(raw_input) / board_dim)
) |>
    aperm(c(2, 1, 3))

winner_board <- solve_part1(boards, board_dim)
answer1 <- sum(Re(winner_board[[1]][Im(winner_board[[1]]) == 0])) * winner_board[[2]]

print(paste("Answer 1:", answer1))

result <- solve_part2(boards, board_dim, numbers)
