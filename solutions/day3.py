...
# Transpose list
processed = list(zip(*[list(x) for x in raw_input]))

counts = list(map(Counter, processed))
evens = [x["0"] == x["1"] for x in counts]
most_common = (k for sub in counts for k, v in sub.items() if v == max(sub.values()))
...
nums = [int(num, base=2) for num in raw_input]
answer2 = 1
