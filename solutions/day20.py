...
def pad(mat, val):
    ...
    return [pad_row(len(mat[0]), val)] + [pad_col(row, val) for row in mat] + [pad_row(len(mat[0]), val)]

# Kludge to swap values of outer padding
def switch(mat):
    reverse = {'0' : '1', '1' : '0'}
    return [ [reverse[val] if j in (0, len(mat[0]) - 1) or i in (0, len(mat) - 1) else val for j, val in enumerate(row) ] for i, row in enumerate(mat) ]
...
def convolve(mat, lookup, inc = 0, iterations = 2):
    ...
    for i in range(1, len(mat) -1 ):
        for j in range(1, len(mat[0]) - 1):
            val = ''.join([mat[k][l]   for k in range(i -1, i+2)for l in range(j-1, j +2)]) 
