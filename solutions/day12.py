...
class Cave():
    ...
    def get_paths(self, node = 'start', traversed = None):
        ...
        neighbors = [room for room in self.rooms[node].neighbors if self.rooms[room].big or room not in traversed ]
    ...
    def get_paths2(self, node = 'start', traversed = None, doubled = False ):
        ...
        traversed.append(node)
        self.rooms[node].visited = True
        neighbors = [room for room in self.rooms[node].neighbors if self.rooms[room].big or (room not in traversed or (room != 'start' and not doubled)) ]
        # Exit if dead end (no adjacent big or unvisited small nodes)
        if not neighbors:
            return
        if node == 'end':
